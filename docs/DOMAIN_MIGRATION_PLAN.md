...
> **执行期追加（2.30.0）**：本表是开工时的阶段划分，不自上而下回溯改写。执行期间新增了 **P6（DOM/标识符面）**——它原本登记在本文件 §8 末节与 `docs/DOMAIN.md` §5.1 的「后续可选项」里，后由执行期提升为独立阶段并落地。P6 的范围、门禁影响与结论见 §8 表末行；它与 P1–P5 的差别在于**不新增弃用条目、不改 wire 面**（OpenAPI 快照 diff 仅版本号一行）。
