...
来源/策略库/CRM 外部系统必须可替换、可故障注入，且外部故障不能破坏审核单正确性。直接内联 HTTP 调用会让每个集成重复错误处理。

## 决策

- `app/connectors.py` 定义 `OrderConnector`/`KnowledgeConnector`/`CRMConnector` Protocol + 沙盒实现（数据来自应用自身存储，身份绑定、跨提交方不泄露）。
