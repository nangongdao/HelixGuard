...
| **Order 连接器** | 熔断打开 → `unavailable` → 升级人工,不泄露状态 | 订单查询转人工复核 | 熔断冷却后半开探针自动恢复 | `OrderUnavailableTests`;`OrchestratorFaultInjectionTests` |
| **CRM 连接器** | `unavailable` → 升级人工,不查订单,留 `customers.resolve` 审计 | 未绑定提交方订单转人工复核 | 同上 | `test_crm_unavailable_escalates_without_order_lookup` |
...
4. **每审核单串行**:同审核单同时最多一个 processing job(claim 子查询)。
