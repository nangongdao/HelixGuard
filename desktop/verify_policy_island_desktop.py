"""Desktop-shell real-machine verification for the policy island editor (D3).

Launches the release helix-desktop.exe with a WebView2 remote-debugging port,
drives the policy view through CDP, and asserts the island (not legacy)
...
DEBUG_PORT = 9333
ISLAND_SELECTORS = {
    "island_root": "#policyReactIsland .policy-island",
    "island_editor_aside": "#policyReactIsland .policy-editor",
    "island_editor_form": "#policyFormReact",
    "island_editor_title": "#policyTitleReact",
    "island_editor_tags": "#policyTagsReact",
    "legacy_editor_hidden": "#policyEditor",
    "legacy_list_hidden": "#policyList",
}
...
def main() -> int:
    ...
    try:
        ...
        with sync_playwright() as playwright:
            ...
            checks["identity"] = page.text_content("#reviewerIdentity") or ""

            # Open the policy view. The island query already fetched on
            ...
            checks["legacy_list_hidden"] = page.evaluate(
                "() => document.querySelector('#policyList').hidden === true"
                " || document.querySelector('#policyList').offsetParent === null"
            )

            # Writer journey inside the island editor: open, fill, submit.
            run_id = uuid4().hex[:6]
            title = f"桌面岛编辑器验证 {run_id}"
            content = f"这是桌面壳 policy 岛编辑器的真机写旅程验证，运行标识 {run_id}。"
            page.wait_for_selector("#policyReactIsland .policy-list .policy-action", timeout=15000)
            ...
            checks["island_editor_opens_blank"] = page.evaluate(
                "() => document.querySelector('#policyTitleReact').value === ''"
            )
            page.fill("#policyTitleReact", title)
            page.fill("#policyContentReact", content)
            page.fill("#policyTagsReact", "desktop, island-verify")
            page.fill("#policySourceReact", "internal:desktop/verify")
            with page.expect_response(
                lambda r: "/api/policy/drafts" in r.url and r.request.method == "POST"
            ):
                page.locator("#policyFormReact button[type='submit']").click()
            page.wait_for_timeout(1200)
            checks["draft_saved_toast"] = page.evaluate(
                "() => Boolean(document.querySelector('.toast'))"
                " || !document.querySelector('#policyFormReact')"
            )
            checks["editor_closed_after_save"] = page.evaluate(
                "() => { const a = document.querySelector('#policyReactIsland .policy-editor');"
                " return a && a.hidden === true; }"
            )
            checks["island_title_visible_after_save"] = page.evaluate(
                f"() => document.querySelector('#policyReactIsland .policy-island')"
            ...
            if failed:
                print(f"FAIL: {failed}")
                return 1
            print("PASS: desktop policy island editor verified end-to-end")
            return 0
    finally:
        proc.terminate()
        try:
