"""QA spot-check rating endpoint (backlog item).

A one-time, unauthenticated rating link issued when a review case is decided.
The token is random and single-use; submitting a rating reflows into the
feedback system (and thus the supervisor quality dashboard).

The link points at ``GET /api/qa-spot-check/{token}``, which renders a minimal
self-contained HTML form the submitter can rate from a browser; the form
POSTs the rating back to ``POST /api/qa-spot-check/{token}``. Both paths share
the same atomic single-use write, so a token can never accept two ratings.

The previous prefix was ``/api/csat``; it keeps serving as a deprecated alias
(``docs/API_POLICY.md`` §2) until the sunset registered in
...
router = APIRouter(tags=["qa_spot_check"])
...
@router.get("/api/qa-spot-check/{token}", response_class=HTMLResponse, include_in_schema=False)
@legacy_route(
    router,
    "/api/csat/{token}",
    methods=["GET"],
...
def csat_landing_page(request: Request, token: str) -> HTMLResponse:
    """Render a minimal submitter survey form (no API key required).
    ...
    The form posts back to whatever path the visitor arrived on, so a link
    issued before the rename keeps its own (deprecated) path end to end.
    """
...
<form method="post" action="/api/qa-spot-check/{token}">
...
@router.post("/api/qa-spot-check/{token}", status_code=200, response_model=None)
@legacy_route(router, "/api/csat/{token}", methods=["POST"], status_code=200, response_model=None)
