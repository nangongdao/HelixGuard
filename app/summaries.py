...
(``unconfigured``) in ``review_case_summaries``.
"""
...
DISPOSITION_SYSTEM_PROMPT = (
    ...
    "conversation: the issue, the verdict reached, and any follow-up the "
...
class SummaryService:
    ...
    def generate(
        ...
        """
        existing = self.database.get_conversation_summary(tenant_id, review_case_id, kind)
        if existing:
            return existing
        content, source = self._summarize(tenant_id, review_case_id, kind)
        return self.database.upsert_conversation_summary(
            tenant_id, review_case_id, kind, content, source
        )
    ...
    def _summarize(self, tenant_id: str, review_case_id: str, kind: str) -> tuple[str, str]:
        ...
        if self.model_provider is None:
            ...
            return self._rule_summary(review_case, messages), OUTCOME_UNCONFIGURED
        decision = self._decision(tenant_id)
        if decision is not None and not decision.allowed:
            return self._rule_summary(review_case, messages), OUTCOME_DENIED
        try:
            content, response = self._model_summary(kind, review_case, messages, tenant_id)
        ...
        return self._rule_summary(review_case, messages), OUTCOME_FAILED
    ...
    def _metadata(self, review_case: dict[str, Any]) -> dict[str, Any]:
        return {
            ...
            "handoff_reason": review_case.get("handoff_reason"),
            "message_count": review_case.get("message_count"),
            "first_response_at": review_case.get("first_response_at"),
            "created_at": review_case.get("created_at"),
        }

    def _rule_summary(self, review_case: dict[str, Any], messages: list[dict[str, Any]]) -> str:
        ...
        msg_count = review_case.get("message_count", len(messages))
        header = (
            f"提交方 {review_case.get('customer_name') or '(匿名)'}"
            f" · 渠道 {review_case.get('channel') or '未知'}"
            f" · 优先级 {review_case.get('priority') or 'normal'}"
            f" · 风险类别 {review_case.get('intent') or '未知'}"
            f" · {msg_count} 条消息"
        )
        if review_case.get("handoff_reason"):
            header += f" · 转人工复核原因: {review_case['handoff_reason']}"
