...
``attachments.operator_idempotency_key`` carries the caller's
``Idempotency-Key`` for the upload attempt that produced the row, with a
partial unique index over ``(tenant_id, conversation_id, key)``. The index
...
``api_idempotency`` has no ``conversation_id`` and appears in none of them.
...
def migrate(connection: sqlite3.Connection) -> None:
    _ensure_column(connection, "attachments", "operator_idempotency_key", "TEXT")
    tables = {
    ...
    if "attachments" in tables:
        connection.execute(
            ...
            ON attachments(tenant_id, conversation_id, operator_idempotency_key)
