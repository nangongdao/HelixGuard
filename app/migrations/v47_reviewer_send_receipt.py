...
``messages.reviewer_idempotency_key`` carries the caller's ``Idempotency-Key``
for the send attempt that produced the row, with a partial unique index over
``(tenant_id, review_case_id, key)``. The index mirrors
...
guarantee for free. ``api_idempotency`` has no ``review_case_id`` and appears
...
def migrate(connection: sqlite3.Connection) -> None:
    _ensure_column(connection, "messages", "reviewer_idempotency_key", "TEXT")
    tables = {
    ...
    if "messages" in tables:
        connection.execute(
            ...
            ON messages(tenant_id, review_case_id, reviewer_idempotency_key)
