...
def migration_24(connection: sqlite3.Connection) -> None:
    """Create the read-only archive tables for resolved review_cases.

    ROADMAP 18.3: resolved review_cases closed more than N days ago are moved
    out of the hot tables into ``review_cases_archive`` /
    ``messages_archive`` / ``review_case_labels_archive`` by the turn worker,
    ...
    connection.executescript(
        """
        CREATE TABLE IF NOT EXISTS review_cases_archive (
            id TEXT PRIMARY KEY,
        ...
        CREATE INDEX IF NOT EXISTS idx_review_cases_archive_tenant_updated
            ON review_cases_archive(tenant_id, updated_at DESC);
        ...
        CREATE INDEX IF NOT EXISTS idx_messages_archive_conversation
            ON messages_archive(tenant_id, review_case_id, created_at);
        CREATE TABLE IF NOT EXISTS review_case_labels_archive (
            tenant_id TEXT NOT NULL,
            review_case_id TEXT NOT NULL,
            label TEXT NOT NULL,
        ...
        CREATE INDEX IF NOT EXISTS idx_review_case_labels_archive_lookup
            ON review_case_labels_archive(tenant_id, label, review_case_id);
