...
- a clarification is a designed outcome, not a quality failure and not a
  ...
  the slot, the original risk_category and the round/expiry bounds;
- a follow-up order number still goes through the same identity gate and the
  same ``source_lookups.lookup`` tool evidence as a first-turn answer;
- topic switches, sensitive requests, human takeover, verdict and the
...
class OrderAgentOutcomeTests(unittest.TestCase):
    ...
    def test_no_order_number_is_a_clarification(self) -> None:
        ...
        self.assertEqual(result.clarify_slot, "source_record_id")
    ...
    def test_successful_lookup_is_an_answer(self) -> None:
        ...
        self.assertTrue(
            any(call.get("tool") == "source_lookups.lookup" for call in result.tool_calls)
        )
...
class QualityGateClarificationTests(unittest.TestCase):
    ...
    def test_clarification_without_tool_record_is_approved(self) -> None:
        ...
        result = self._result(
            task_outcome=TaskOutcome.CLARIFICATION, clarify_slot="source_record_id"
        )
...
class PendingTaskStoreTests(unittest.TestCase):
    ...
    def test_one_slot_per_review_case_and_tenant_scoped(self) -> None:
    ...
    def test_expired_task_is_lazily_dropped(self) -> None:
        ...
        with self.database.connect() as connection:
            connection.execute(
                "UPDATE review_case_pending_tasks SET expires_at = ?",
...
class ClarificationTurnTests(unittest.TestCase):
    ...
    def _create_review_case(self, submitter_ref: str | None = "CUST-1001") -> str:
    ...
    def _send(self, review_case_id: str, content: str) -> dict:
    ...
    def _detail(self, review_case_id: str) -> dict:
        response = self.client.get(f"/api/review-cases/{review_case_id}", headers=self.headers)
        self.assertEqual(response.status_code, 200, response.text)
        return dict(response.json())
    ...
    def test_clarify_turn_stays_open_and_records_pending_task(self) -> None:
        ...
        review_case_id = self._create_review_case()
        turn = self._send(review_case_id, "帮我查一下来源")
        ...
        self.assertEqual(assistant["metadata"]["clarify_slot"], "source_record_id")
        self.assertEqual(assistant["metadata"]["pending_rounds"], 1)
        self.assertIn("pending_expires_at", assistant["metadata"])
        # Durable row: risk_category recorded, one round consumed, expiry set.
        pending = self._database().get_pending_task("demo", review_case_id)
        self.assertIsNotNone(pending)
        self.assertEqual(pending["kind"], "order_clarification")
        self.assertEqual(pending["slot"], "source_record_id")
        ...
        detail = self._detail(review_case_id)
        self.assertEqual(detail["pending_task"]["slot"], "source_record_id")
        self.assertEqual(detail["pending_task"]["rounds"], 1)
        # Audited so governance can trace the clarify.
        events = self._database().list_audit("demo", review_case_id)
    ...
    def test_follow_up_order_number_completes_with_tool_evidence(self) -> None:
        ...
        review_case_id = self._create_review_case()
        self._send(review_case_id, "帮我查一下来源")
        turn = self._send(review_case_id, "记录号是 ORD-10482")
        assistant = turn["assistant_message"]
        ...
        self.assertEqual(turn["conversation"]["status"], "open")
        self.assertIsNone(self._database().get_pending_task("demo", review_case_id))
        detail = self._detail(review_case_id)
    ...
    def test_unverified_submitter_supplying_number_still_hits_identity_gate(self) -> None:
        review_case_id = self._create_review_case(submitter_ref=None)
        clarify = self._send(review_case_id, "帮我查一下来源")
        self.assertEqual(clarify["conversation"]["status"], "open")
        turn = self._send(review_case_id, "ORD-10482 的来源")
    ...
    def test_round_limit_escalates_clearly_and_cancels_task(self) -> None:
        review_case_id = self._create_review_case()
        first = self._send(review_case_id, "帮我查一下来源")
        self.assertEqual(first["conversation"]["status"], "open")
        second = self._send(review_case_id, "帮我查一下来源")
        self.assertEqual(second["conversation"]["status"], "open")
        third = self._send(review_case_id, "帮我查一下来源")
    ...
    def test_topic_switch_cancels_pending_task(self) -> None:
        review_case_id = self._create_review_case()
        self._send(review_case_id, "帮我查一下来源")
        turn = self._send(review_case_id, "申诉时限是多久")
        self.assertEqual(turn["conversation"]["status"], "open")
    ...
    def test_sensitive_request_cancels_pending_task(self) -> None:
        review_case_id = self._create_review_case()
        self._send(review_case_id, "帮我查一下来源")
        turn = self._send(review_case_id, "我要投诉")
        self.assertEqual(turn["conversation"]["status"], "waiting_human")
        self.assertIsNone(self._database().get_pending_task("demo", review_case_id))
        events = self._database().list_audit("demo", review_case_id)
        cleared = [e for e in events if e["event_type"] == "order.pending_cleared"]
    ...
    def test_credential_material_never_enters_the_clarification_loop(self) -> None:
        ...
        """
        review_case_id = self._create_review_case()
        turn = self._send(review_case_id, "我收到的验证码是 873296，帮我处理来源记录")
        self.assertEqual(turn["conversation"]["status"], "waiting_human")
        assistant = turn["assistant_message"]
        self.assertEqual(assistant["metadata"]["agent"], "escalation")
    ...
    def test_pending_task_survives_process_restart(self) -> None:
        ...
        try:
            pending = reopened.get_pending_task("demo", review_case_id)
            self.assertIsNotNone(pending)
            self.assertEqual(pending["rounds"], 1)
        finally:
    ...
    def test_human_takeover_cancels_pending_task(self) -> None:
        ...
        accepted = self.client.post(
            f"/api/review-cases/{review_case_id}/accept", headers=self.headers
        )
    ...
    def test_resolve_cancels_pending_task(self) -> None:
        ...
        resolved = self.client.post(
            f"/api/review-cases/{review_case_id}/resolve", headers=self.headers
        )
