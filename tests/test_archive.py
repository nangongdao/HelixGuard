...
review_cases out of the hot tables, the transparent read merge (get /
    ...
    def test_archive_tables_created(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            try:
                ...
                self.assertIn("review_cases_archive", tables)
                self.assertIn("messages_archive", tables)
                self.assertIn("review_case_labels_archive", tables)
...
class ArchiveDatabaseTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.old_conv = self.db.create_review_case("t1", "Old Customer", None, "web", "admin", 120)
        self.recent_conv = self.db.create_review_case(
            "t1", "Recent Customer", None, "web", "admin", 120
        )
        self.open_conv = self.db.create_review_case(
    ...
    def _resolve(self, review_case_id: str, decided_at: str) -> None:
        with self.db.connect() as connection:
            connection.execute(
                "UPDATE review_cases SET status = 'resolved', decided_at = ?, updated_at = ? "
                "WHERE id = ?",
                (decided_at, decided_at, review_case_id),
            )

    def _add_label(self, review_case_id: str, label: str) -> None:
        with self.db.connect() as connection:
            connection.execute(
                "INSERT INTO review_case_labels (tenant_id, review_case_id, label, "
                "created_by, created_at) VALUES (?, ?, ?, ?, ?)",
                ("t1", review_case_id, label, "admin", utc_now()),
            )

    def test_archive_moves_only_old_resolved_review_cases(self) -> None:
        count = self.db.archive_decided_review_cases(self.cutoff, max_count=10)
        ...
        with self.db.connect() as connection:
            hot = connection.execute(
                "SELECT 1 FROM review_cases WHERE id = ?", (self.old_conv["id"],)
            ).fetchone()
        self.assertIsNone(hot)
        via_merge = self.db.get_review_case("t1", self.old_conv["id"])
        ...
        archived = self.db.get_archived_review_case("t1", self.old_conv["id"])
        self.assertIsNotNone(archived)
        assert archived is not None
        self.assertEqual(archived["customer_name"], "Old Customer")

        # Recent resolved and open review_cases must stay hot.
        self.assertIsNotNone(self.db.get_review_case("t1", self.recent_conv["id"]))
        self.assertIsNotNone(self.db.get_review_case("t1", self.open_conv["id"]))

    def test_messages_and_labels_are_moved_with_the_review_case(self) -> None:
        self.db.archive_decided_review_cases(self.cutoff, max_count=10)
    ...
    def test_list_review_cases_archived_flag(self) -> None:
        self.db.archive_decided_review_cases(self.cutoff, max_count=10)
        hot = self.db.list_review_cases("t1", sort="priority", limit=50)
        self.assertEqual({r["id"] for r in hot}, {self.recent_conv["id"], self.open_conv["id"]})
        archived = self.db.list_review_cases("t1", sort="updated", limit=50, archived=True)
        self.assertEqual({r["id"] for r in archived}, {self.old_conv["id"]})

    def test_archived_list_search_falls_back_to_like(self) -> None:
        self.db.archive_decided_review_cases(self.cutoff, max_count=10)
        rows = self.db.list_review_cases("t1", search="Old", limit=50, archived=True)
    ...
    def test_feedback_moved_with_review_case(self) -> None:
        ...
        with self.db.connect() as connection:
            connection.execute(
                "INSERT INTO feedback (id, tenant_id, review_case_id, message_id, actor, "
        ...
        with self.db.connect() as connection:
            hot = connection.execute(
                "SELECT COUNT(*) AS n FROM feedback WHERE review_case_id = ?",
                (self.old_conv["id"],),
            ).fetchone()
            cold = connection.execute(
                "SELECT COUNT(*) AS n FROM feedback_archive WHERE review_case_id = ?",
                (self.old_conv["id"],),
            ).fetchone()
        self.assertEqual(int(hot["n"]), 0)
        self.assertEqual(int(cold["n"]), 1)

    def test_archive_skips_review_case_with_active_job(self) -> None:
        self.db.enqueue_turn_job("t1", self.old_conv["id"], "queue-key", "actor", "内容", 3)
        count = self.db.archive_decided_review_cases(self.cutoff, max_count=10)
        self.assertEqual(count, 0)
        self.assertIsNotNone(self.db.get_review_case("t1", self.old_conv["id"]))

    def test_archive_is_bounded_and_idempotent(self) -> None:
        self.assertEqual(self.db.archive_decided_review_cases(self.cutoff, max_count=1), 1)
        # Nothing left to move on a replay, and the batch cap still respected.
        self.assertEqual(self.db.archive_decided_review_cases(self.cutoff, max_count=10), 0)
...
class ArchiveWorkerTests(unittest.TestCase):
    ...
    def test_worker_archives_and_counts(self) -> None:
        ...
        conv = database.create_review_case("t1", "Archived Worker", None, "web", "admin", 120)
        ...
        with database.connect() as connection:
            hot = connection.execute(
                "SELECT 1 FROM review_cases WHERE id = ?", (conv["id"],)
            ).fetchone()
        self.assertIsNone(hot)
        via_merge = database.get_review_case("t1", conv["id"])
        self.assertIsNotNone(via_merge)
        assert via_merge is not None
        self.assertIsNotNone(via_merge.get("archived_at"))
        self.assertIsNotNone(database.get_archived_review_case("t1", conv["id"]))
