...
def test_normalize_basic():
    result = normalize_review_case_labels(["Bug", "Feature", "urgent"])
    assert result == ["bug", "feature", "urgent"]


def test_normalize_strips_whitespace():
    result = normalize_review_case_labels(["  Bug  ", "Feature\t", "\nurgent "])
    assert result == ["bug", "feature", "urgent"]


def test_normalize_deduplicates_case_insensitive():
    result = normalize_review_case_labels(["Bug", "BUG", "bug", "Feature"])
...
def test_normalize_empty_labels_rejected():
    with pytest.raises(ValueError, match="labels must be printable"):
        normalize_review_case_labels(["Bug", "   ", "Feature"])


def test_normalize_too_long_rejected():
    long_label = "a" * 33
    with pytest.raises(ValueError, match="labels must be printable"):
        normalize_review_case_labels([long_label])


def test_normalize_non_printable_rejected():
    with pytest.raises(ValueError, match="labels must be printable"):
        normalize_review_case_labels(["Bug", "Feature\x00", "urgent"])
