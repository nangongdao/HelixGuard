...
class _SandboxFixture:
    def __init__(self) -> None:
        ...
        self.db.initialize()  # seeds the demo source_lookups and knowledge articles
...
class OrderConnectorConformanceMixin(_Assertions):
    ...
    ``(connector, known_submitter_ref, known_source_record_id, known_order_status)``.
    ...
    def test_identity_required_without_submitter_ref(self) -> None:
    ...
    def test_verified_submitter_succeeds(self) -> None:
    ...
    def test_other_submitter_does_not_disclose(self) -> None:
        conn, _cust, oid, _status = self.make_order_connector()
...
class _ConformanceTransport:
    ...
    def __init__(self, known_customer: str, known_order: str, status: str) -> None:
        self.known_submitter = known_customer
    ...
    def __call__(
        ...
        source_record_id = url.rsplit("/source_lookups/", 1)[-1].split("?")[0]
        submitter = params.get("customer_ref", "")
        if source_record_id != self.known_order or submitter != self.known_submitter:
            return 404, {}
        return 200, {
            "order": {
                "id": source_record_id,
...
class CRMConnectorConformanceMixin(_Assertions):
    ...
    ``(connector, known_submitter_ref, known_name)``.
    """

    def make_crm_connector(self) -> tuple[CRMConnector, str, str]:
        raise NotImplementedError

    def test_known_submitter_resolves(self) -> None:
        conn, submitter_ref, name = self.make_crm_connector()
        result = conn.resolve_customer("demo", submitter_ref)
        self.assertTrue(result.ok)
        self.assertEqual(result.code, "ok")
        assert result.profile is not None
        self.assertEqual(result.profile.submitter_ref, submitter_ref)
        self.assertEqual(result.profile.name, name)

    def test_unknown_submitter_not_found(self) -> None:
        conn, _submitter_ref, _name = self.make_crm_connector()
...
class _CRMConformanceTransport:
    ...
    def __call__(
        ...
        submitter = url.rsplit("/customers/", 1)[-1].split("?")[0]
        if submitter != self.known_submitter:
            return 404, {}
        return 200, {"customer_ref": submitter, "name": self.name}
...
class SandboxKnowledgeAndCRMTests(unittest.TestCase):
    ...
    def test_crm_resolves_known_and_unknown_submitter(self) -> None:
        result = self.crm_connector.resolve_customer("demo", "CUST-1001")
        self.assertTrue(result.ok)
        assert result.profile is not None
        self.assertEqual(result.profile.submitter_ref, "CUST-1001")
