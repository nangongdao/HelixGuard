...
reply, a resolution and the CSAT link all arrive *without* the customer sending
...
class _WidgetFixture(unittest.TestCase):
    ...
    def _add(self, conversation_id: str, role: str, content: str) -> str:
        message = self.db.add_message(self.tenant_id, conversation_id, role, "fixture", content)
...
class WidgetMessageCursorTests(_WidgetFixture):
    ...
    def test_page_exposes_cursor_so_a_client_can_resume(self) -> None:
        conversation_id, token = self._session("Cursor")
        self._add(conversation_id, "customer", "first")

        status, headers, body = self._list(conversation_id, token)
    ...
    def test_cursor_returns_only_the_messages_that_arrived_since(self) -> None:
        conversation_id, token = self._session("Delta")
        self._add(conversation_id, "customer", "first")
        _, headers, first_page = self._list(conversation_id, token)
        cursor = headers["X-Next-Cursor"]

        # The operator answers while the customer sits idle.
        reply_id = self._add(conversation_id, "operator", "人工答复")
        status, delta_headers, delta = self._list(
            conversation_id, token, f"?cursor={quote(cursor)}"
        )
    ...
    def test_invalid_cursor_is_rejected_with_400(self) -> None:
        conversation_id, token = self._session("Bad")
        status, _headers, _body = self._list(conversation_id, token, "?cursor=not-a-real-cursor")
    ...
    def test_a_page_of_internal_notes_does_not_stall_the_cursor(self) -> None:
        ...
        status, _headers, second = self._list(
            conversation_id, token, f"?limit=2&cursor={quote(headers['X-Next-Cursor'])}"
        )
        self.assertEqual(status, 200)
        self.assertEqual([message["id"] for message in second], [visible_id])

    def test_cursor_cannot_reach_another_conversation(self) -> None:
    ...
    def test_idle_poll_carries_reply_and_resolution_without_a_customer_send(self) -> None:
        """H01 headline: the customer sends nothing and still learns everything."""
        conversation_id, token = self._session("Idle")
        self._add(conversation_id, "customer", "我的订单到哪了")
        _status, headers, _body = self._list(conversation_id, token)
        cursor = headers["X-Next-Cursor"]

        # From here on the customer sends nothing at all.
        reply_id = self._add(conversation_id, "operator", "已为你加急处理")
        ...
        status, poll_headers, delta = self._list(
            conversation_id, token, f"?limit=50&cursor={quote(cursor)}"
        )
        self.assertEqual(status, 200)
        self.assertEqual([message["id"] for message in delta], [reply_id])
    ...
    def test_an_empty_delta_still_reports_the_conversation_status(self) -> None:
        """No new rows is not "no news": a resolve without a reply must show."""
        conversation_id, token = self._session("Quiet")
        self._add(conversation_id, "customer", "还有人在吗")
        _status, headers, _body = self._list(conversation_id, token)

        status, poll_headers, delta = self._list(
            conversation_id, token, f"?limit=50&cursor={quote(headers['X-Next-Cursor'])}"
        )
        self.assertEqual(status, 200)
    ...
    def test_limit_still_caps_the_visible_page(self) -> None:
        conversation_id, token = self._session("Limit")
        for index in range(5):
            self._add(conversation_id, "operator", f"reply {index}")

        status, headers, body = self._list(conversation_id, token, "?limit=3")
        self.assertEqual(status, 200)
        self.assertEqual(len(body), 3)
        self.assertEqual(headers.get("X-Has-More"), "true")


class WidgetInternalRoleProjectionTests(_WidgetFixture):
    """Internal roles must not reach the customer's browser."""

    def test_every_internal_role_is_hidden_from_the_customer(self) -> None:
        ...
        conversation_id, token = self._session("Projection")
        for role in sorted(INTERNAL_MESSAGE_ROLES):
            self._add(conversation_id, role, f"{role} 内部内容")
        visible_id = self._add(conversation_id, "operator", "公开答复")

        status, _headers, body = self._list(conversation_id, token, "?limit=50")
        self.assertEqual(status, 200)
        self.assertEqual([message["id"] for message in body], [visible_id])

    def test_unknown_roles_stay_visible_to_the_customer(self) -> None:
        ...
        conversation_id, token = self._session("Unknown")
        unknown_id = self._add(conversation_id, "assistant_draft", "hello")
